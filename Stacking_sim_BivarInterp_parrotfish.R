...
sim_dir <- file.path(main_dir, "sim_parrotfish")
...
##################################
sp = Plot_taxa( Taxa=Search_species(Genus="Scarus",Species="rubroviolaceus")$match_taxonomy, mfrow=c(2,2) )
...
png(file.path(fig_dir, "4param_2D_Parrotfish.png"), height=10, width=11, units="in", res=200)
...
png(file.path(fig_dir, "4param_FishLifeDefault_Parrotfish.png"), height=10, width=11, units="in", res=200)
param <- c("M","K","Loo","Lm")
par(mfrow=c(4,4), mar=c(0,0,0,0), omi=c(0.5,0.5,0.5,0.5))
...
pconverge <- ggplot(cover %>% filter(Variable=="SPR")) +
    ...
    theme(axis.text=element_text(size=12),
        axis.title=element_text(size=14)) +
    ylab("Convergence rate") + xlab("Life history approach") +
    ylim(c(0,100))
