...
#########################
re <- rem <- rei <- mean <- var <- tval <- cover <- coverm <- coveri <- rep(NA, length(itervec))
...
if( Dim==2 ){
  ...
  SPRquant = wtd.quantile(SPR_j, w=Prob_j, probs=c(0.25,0.75))
}
  ...
  var[i] <- SPRvar
  tval[i] <- true$SPR
  cover[i] <- ifelse((true$SPR >= SPRquant[1] & true$SPR <= SPRquant[2]), 1, 0)
      mquant <- c(mres@SPR - 0.674*sqrt(mres@Vars[,"SPR"]), mres@SPR + 0.674*sqrt(mres@Vars[,"SPR"]))
  coverm[i] <- ifelse((true$SPR >= mquant[1] & true$SPR <= mquant[2]), 1, 0)
      iquant <- c(ires@SPR - 0.675*sqrt(ires@Vars[,"SPR"]), ires@SPR + 0.674*sqrt(ires@Vars[,"SPR"]))
  coveri[i] <- ifelse((true$SPR >= iquant[1] & true$SPR <= iquant[2]), 1, 0)
}
...
par(mfrow=c(2,1))
si.zmin <- min(Prob_j, na.rm=TRUE)
si.zmax <- max(Prob_j, na.rm=TRUE)
breaks <- pretty(c(si.zmin, si.zmax),10)
colors <- rev(topo.colors(length(breaks)-1))

image(matrix(Prob_j, nrow=100), col=colors)
image(SPR_j, col=colors)

par(mfrow=c(1,1))
for(i in 1:nrow(SPR_j)){
  for(j in 1:ncol(SPR_j)){
