...
Param_i = t( Mean + Eigen$vectors[,1:Dim,drop=FALSE] %*% Diag(sqrt(Eigen$values[1:Dim])) %*% t(Nodes_i) )
...
if( Dim==1 ){
  ...
  # START CHANGE
  interp_h = approx(x=Nodes_i[,1], y=SPR_i, n=100)
  # END CHANGE
  # START CHANGE
  SPR_j <- interp_h$y
  # END CHANGE
...
if( Dim==2 ){
  ...
  # START CHANGE
  interp_h = akima::interp( x=Nodes_i[,1], y=Nodes_i[,2], z=SPR_i, nx=100, ny=100, duplicate="median" )
  # END CHANGE
  SPR_j = interp_h$z

  # Step 3B:  translate Grid to 4 parameters and calculate FishLife probability for each
  # START CHANGE
  Grid_j <- cbind( interp_h$x, interp_h$y )
  # END CHANGE
  Param_j = t( Mean + Eigen$vectors[,1:Dim,drop=FALSE] %*% Diag(sqrt(Eigen$values[1:Dim])) %*% t(Grid_j) )
  colnames(Param_j) = names(Mean)
  Prob_j = dmvnorm( Param_j, mean=Mean, sigma=Cov )
