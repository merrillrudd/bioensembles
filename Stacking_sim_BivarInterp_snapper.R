...
sim_dir <- file.path(main_dir, "sim_snapper")
...
##################################
### Adams and Rios 2015, filtered to what is not in FishBase
lhstudies <- read.csv(file.path(sim_dir, "Snapper_LH_Adams_and_Rios_2015.csv"))
study_vec <- lhstudies$Location
M_vec <- lhstudies$M
loo_vec <- lhstudies$Loo
vbk_vec <- lhstudies$K
Lm_vec <- lhstudies$Lm

lw <- length_weight(species_list = c("Ocyurus chrysurus"))
a_vec <- lw$a
b_vec <- lw$b
...
##################################
sp = Plot_taxa( Taxa=Search_species(Genus="Ocyurus",Species="chrysurus")$match_taxonomy, mfrow=c(2,2) )
...
png(file.path(fig_dir, "4param_2D_Snapper.png"), height=10, width=11, units="in", res=200)
...
png(file.path(fig_dir, "4param_FishLifeDefault_Snapper.png"), height=10, width=11, units="in", res=200)
param <- c("M","K","Loo","Lm")
par(mfrow=c(4,4), mar=c(0,0,0,0), omi=c(0.5,0.5,0.5,0.5))
...
for(i in 1:length(param)){
  for(j in 1:length(param)){
    ...
    if(param[i]!=param[j]){
      ...
      plot(x=1, y=1, type="n", 
        ylim=c(min(c(exp(Param2_i[,param[i]]),yplot),na.rm=TRUE), max(c(exp(Param2_i[,param[i]]),yplot),na.rm=TRUE)), 
        xlim=c(min(c(exp(Param2_i[,param[j]]),xplot),na.rm=TRUE), max(c(exp(Param2_i[,param[j]]),xplot),na.rm=TRUE)),
...
##############################################
## Draw true values of life history parameters
##############################################
set.seed(123)
lh_inp <- rmvnorm(length(itervec), mean=Mean, sigma=Cov)

lh_equil <- lapply(1:nrow(lh_inp), function(x){
  create_lh_list(vbk=exp(lh_inp[x,"K"]), linf=exp(lh_inp[x,"Loo"]),
                 M=exp(lh_inp[x,"M"]),
                 M50=exp(lh_inp[x,"Lm"]), maturity_input="length",
                 lwa=mean(a_vec), lwb=mean(b_vec),
                 S50=exp(lh_inp[x,"Lm"]), S95=min(exp(lh_inp[x,"Loo"])*0.95, exp(lh_inp[x,"Lm"])*1.2), selex_input="length",
                 SigmaF=0.001, SigmaR=0.001, rho=0, 
                 binwidth=1, theta=10)
})

lh_var <- lapply(1:nrow(lh_inp), function(x){
  create_lh_list(vbk=exp(lh_inp[x,"K"]), linf=exp(lh_inp[x,"Loo"]),
                 M=exp(lh_inp[x,"M"]),
                 M50=exp(lh_inp[x,"Lm"]), maturity_input="length",
                 lwa=mean(a_vec), lwb=mean(b_vec),
                 S50=exp(lh_inp[x,"Lm"]), S95=min(exp(lh_inp[x,"Loo"])*0.95, exp(lh_inp[x,"Lm"])*1.2), selex_input="length",
                 SigmaF=0.1, SigmaR=0.737, rho=0.4, 
                 binwidth=1, theta=10)
})
...
plot(x=1,y=1,type="n", cex.axis=1.2, ylim=c(0,3), xlim=c(1,20), xaxt="n")
for(i in 1:length(itervec)){
  true <- readRDS(file.path(equil_dir, itervec[i], "True.rds"))
...
plot(x=1,y=1,type="n", cex.axis=1.2, ylim=c(0,3), xlim=c(1,20), xaxt="n", yaxt="n")
for(i in 1:length(itervec)){
  true <- readRDS(file.path(var_dir, itervec[i], "True.rds"))
  lines(true$F_ft[1,], type="l", col="#AAAAAA80")
}
...
foreach(loop=itervec, .packages=c('TMB','LIME','LBSPR')) %dopar%
  run_single(modpath=equil_dir, iter=loop, lh=lh_equil[[loop]], model="LBSPR", name="BestCase")
end <- Sys.time() - start
stopCluster(cl)
...
start <- Sys.time()
foreach(loop=itervec, .packages=c('TMB','LIME','LBSPR')) %dopar%
  run_single(modpath=equil_dir, iter=loop, lh=lh_equil[[loop]], model="LIME", name="BestCase")
end <- Sys.time() - start
stopCluster(cl)
...
start <- Sys.time()
foreach(loop=itervec, .packages=c('TMB','LIME','LBSPR')) %dopar%
  run_single(modpath=var_dir, iter=loop, lh=lh_var[[loop]], model="LBSPR", name="BestCase")
end <- Sys.time() - start
stopCluster(cl)
...
start <- Sys.time()
foreach(loop=itervec, .packages=c('TMB','LIME','LBSPR')) %dopar%
  run_single(modpath=var_dir, iter=loop, lh=lh_var[[loop]], model="LIME", name="BestCase")
end <- Sys.time() - start
stopCluster(cl)
...
lh_means <-   with(lh_var[[1]], create_lh_list(vbk=exp(Mean["K"]), linf=exp(Mean["Loo"]),
                 M=exp(Mean["M"]),
  ...
  byApproach <- lapply(1:length(bivar_approach), function(aa){
    byIter <- lapply(1:length(itervec), function(ii){
      ...
      byMod <- lapply(1:length(models), function(mm){
        ...
        avgmean <- sapply(1:nrow(y_i), function(x) mean(y_i[x,]))
        avgsd <- sapply(1:nrow(ysd_i), function(x) mean(ysd_i[x,]))
...
summary <- res_all3 %>% 
          ...
          mutate(Converge = ifelse(is.na(MLE), 0, 1)) %>%
...
re <- summary %>% filter(Year==max(Year, na.rm=TRUE)) %>%
...
pre <- ggplot(summary %>% filter(Variable=="SPR") %>% filter(Year==max(Year, na.rm=TRUE))) +
